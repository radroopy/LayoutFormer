...
def _sample_sdf(sdf_maps: torch.Tensor, grid: torch.Tensor) -> torch.Tensor:
...
def compute_losses(
    ...
    grid_x = pred_xy_norm[..., 0] * x_scale.view(-1, 1) * 2 - 1       #映射到grid_sample期望的[-1,1]
    grid_y = (1 - pred_xy_norm[..., 1]) * y_scale.view(-1, 1) * 2 - 1
    grid_pred = torch.stack([grid_x, grid_y], dim=-1).unsqueeze(2)  # (B, N, 1, 2)
    sampled_pred = _sample_sdf(sdf_maps, grid_pred)

    # GT centers -> determine which elements are inside the boundary
    gt_xy_norm = gt_xy / NORM_RANGE
    grid_x = gt_xy_norm[..., 0] * x_scale.view(-1, 1) * 2 - 1
    grid_y = (1 - gt_xy_norm[..., 1]) * y_scale.view(-1, 1) * 2 - 1
    grid_gt = torch.stack([grid_x, grid_y], dim=-1).unsqueeze(2)
    sampled_gt = _sample_sdf(sdf_maps, grid_gt)
    inside_mask = (sampled_gt <= 0).to(mask.dtype)
    shape_mask = mask * inside_mask

    l_shape = masked_mean(F.relu(sampled_pred), shape_mask)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Warmup epochs for lr schedule (default: 5.0).",
    )
    parser.add_argument(
        "--min-lr",
        type=float,
    ...
    if args.lr_schedule == "cosine":
        ...
        min_lr_ratio = min(1.0, args.min_lr / max(args.lr, 1e-12))
